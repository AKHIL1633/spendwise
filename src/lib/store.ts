...
export const memDb = {
  ...
  create(payload: Omit<Transaction, "id"|"createdAt"|"updatedAt">, userId: string): Transaction {
    const t: Transaction = { ...payload, id: uuidv4(), userId, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  ...
  getStats(userId: string): DashboardStats {
    ...
    return { totalIncome, totalExpense, netBalance: totalIncome - totalExpense, transactions: userTx.length };
  },
